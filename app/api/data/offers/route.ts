...
type ImplantSetRow = { type_id: number; name_en: string; name_zh: string | null; buy_price: number; sell_price: number; lp_cost: number; isk_cost: number; material_cost: number };
...
export async function GET(request: Request) {
  ...
  if (corporationId && url.searchParams.get("view") === "detail") {
    ...
    const implantRows = (await db.prepare(`SELECT t.type_id, COALESCE(t.name_en, '') name_en, t.name_zh, o.lp_cost, o.isk_cost, COALESCE(mo.buy_price, 0) buy_price, COALESCE(mo.sell_price, 0) sell_price, COALESCE((SELECT SUM(m.quantity * COALESCE(mo2.sell_price, 0)) FROM lp_offer_materials m LEFT JOIN market_orders mo2 ON mo2.region_id=? AND mo2.type_id=m.type_id WHERE m.corporation_id=o.corporation_id AND m.offer_id=o.offer_id), 0) material_cost FROM lp_offers o JOIN item_types t ON t.type_id=o.type_id LEFT JOIN market_orders mo ON mo.region_id=? AND mo.type_id=t.type_id WHERE o.corporation_id=? AND (t.name_en LIKE '% Alpha' OR t.name_en LIKE '% Beta' OR t.name_en LIKE '% Gamma' OR t.name_en LIKE '% Delta' OR t.name_en LIKE '% Epsilon' OR t.name_en LIKE '% Omega')`).bind(MARKET_REGION_ID, MARKET_REGION_ID, corporationId).all<ImplantSetRow>()).results;
    ...
    const implantSets = [...grouped.entries()].filter(([, set]) => set.size === 6).map(([name, set]) => {
      ...
      const netProfit = items.every(item => item.buy_price > 0) ? items.reduce((sum, item) => sum + item.buy_price * 0.97 - item.isk_cost - item.material_cost, 0) : null;
      return { name: chineseName || name, item_count: 6, total_lp: totalLp, lp_ratio: netProfit !== null && totalLp > 0 ? netProfit / totalLp : null, priced_count: items.filter(item => item.sell_price > 0).length, market_value: marketValue, items: items.map((item, index) => ({ level: levels[index], type_id: item.type_id, name: item.name_zh || item.name_en, lp_cost: item.lp_cost, sell_price: item.sell_price })) };
    });
