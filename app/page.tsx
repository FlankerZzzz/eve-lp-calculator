...
type OptimizationResult = { lines: OptimizationLine[]; usedLp: number; remainingLp: number; totalValue: number; totalProfit: number; lpRatio: number; exact: boolean };
...
export default function Home() {
  ...
  const optimizeCorporation = useCallback(async () => {
    ...
    try {
      ...
      const totalValue = lines.reduce((sum, line) => sum + line.offer.buy_price * line.offer.revenue_quantity * line.count, 0);
      const totalProfit = lines.reduce((sum, line) => sum + line.offer.minProfit! * line.count, 0);
      const lpRatio = usedLp > 0 ? totalProfit / usedLp : 0;
      setOptimization({ lines, usedLp, remainingLp: lp - usedLp, totalValue, totalProfit, lpRatio, exact });
    } catch (error) {
  ...
  const corporationCalculator = <><section className="control-card compact corporation-calculator"><div className="control-title"><span className="step">01</span><div><b>当前企业 LP 组合优化</b><p>使用全部兑换方案计算，不受当前分页影响</p></div></div><label className="amount-label">可用 LP <div className="amount-field"><input type="text" inputMode="numeric" value={formatNumber(lp)} onChange={event => setLp(Math.max(0, Number(event.target.value.replace(/[^0-9]/g, "")) || 0))} /><small>{magnitudeHint(lp)}</small></div><span>LP</span></label><label>需求材料 <strong className="fixed-value">卖单价</strong></label><label>产出物 <strong className="fixed-value">收单价</strong></label><label>交易税 <input type="number" min="0" max="100" value={tax} onChange={event => setTax(Number(event.target.value))} /><span>%</span></label><button className="calculate-button" disabled={optimizing || lp <= 0} onClick={optimizeCorporation}>{optimizing ? "正在计算…" : "计算最优组合"}</button></section>{optimization && <section className="optimization-result"><div className="optimization-summary"><div><small>预计税后总利润</small><strong>{formatNumber(optimization.totalProfit)} ISK</strong><em>{magnitudeHint(optimization.totalProfit)} ISK</em></div><div><small>总体价值（收单）</small><strong>{formatNumber(optimization.totalValue)} ISK</strong><em>{magnitudeHint(optimization.totalValue)} ISK</em></div><div><small>总体 LP 比例</small><strong>{formatNumber(optimization.lpRatio)} ISK/LP</strong><em>税后净利润 ÷ 已使用 LP</em></div><div><small>已使用 LP</small><b>{formatNumber(optimization.usedLp)}</b><em>{magnitudeHint(optimization.usedLp)} LP</em></div><div><small>剩余 LP</small><b>{formatNumber(optimization.remainingLp)}</b><em>{magnitudeHint(optimization.remainingLp)} LP</em></div><span>{optimization.exact ? "精确组合" : "大额 LP：尾部精确优化"}</span></div><div className="optimization-lines">{optimization.lines.map(line => <div key={line.offer.offer_id}><strong className="icon-label"><ItemIcon typeId={line.offer.type_id} name={line.offer.item_name} />{line.offer.item_name}</strong><b>× {formatNumber(line.count)}</b><span>消耗 {formatNumber(line.offer.lp_cost * line.count)} LP<small>{magnitudeHint(line.offer.lp_cost * line.count)}</small></span><span>利润 {formatNumber(line.offer.minProfit! * line.count)} ISK<small>{magnitudeHint(line.offer.minProfit! * line.count)}</small></span></div>)}</div></section>}</>;
