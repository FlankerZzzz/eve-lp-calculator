...
type OptimizationResult = { lines: OptimizationLine[]; usedLp: number; remainingLp: number; totalProfit: number; exact: boolean };
...
type ImplantSet = { name: string; item_count: number; total_lp: number; priced_count: number; market_value: number | null; items: { level: string; type_id: number; name: string; lp_cost: number; sell_price: number }[] };
...
export default function Home() {
  ...
  }, [corporationId, factionId, page, pageSize, sortDirection, sortKey, tax, windowDays]);

  useEffect(() => { loadOptions().catch(error => setStatus(error instanceof Error ? error.message : "读取数据库失败")); }, [loadOptions]);
  useEffect(() => { loadOffers(); }, [loadOffers]);
  useEffect(() => { setOptimization(null); }, [corporationId, lp, tax, windowDays]);
  useEffect(() => {
  ...
  const optimizeCorporation = useCallback(async () => {
    ...
    try {
      ...
      setOptimization({ lines, usedLp, remainingLp: lp - usedLp, totalProfit, exact });
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "组合计算失败");
    } finally { setOptimizing(false); }
  }, [calculate, corporationId, lp, tax, windowDays]);
  ...
  const corporationCalculator = <><section className="control-card compact corporation-calculator"><div className="control-title"><span className="step">01</span><div><b>当前企业 LP 组合优化</b><p>使用全部兑换方案计算，不受当前分页影响</p></div></div><label className="amount-label">可用 LP <div className="amount-field"><input type="text" inputMode="numeric" value={formatNumber(lp)} onChange={event => setLp(Math.max(0, Number(event.target.value.replace(/[^0-9]/g, "")) || 0))} /><small>{magnitudeHint(lp)}</small></div><span>LP</span></label><label>需求材料 <strong className="fixed-value">卖单价</strong></label><label>产出物 <strong className="fixed-value">收单价</strong></label><label>交易税 <input type="number" min="0" max="100" value={tax} onChange={event => setTax(Number(event.target.value))} /><span>%</span></label><button className="calculate-button" disabled={optimizing || lp <= 0} onClick={optimizeCorporation}>{optimizing ? "正在计算…" : "计算最优组合"}</button></section>{optimization && <section className="optimization-result"><div className="optimization-summary"><div><small>预计税后总利润</small><strong>{formatNumber(optimization.totalProfit)} ISK</strong><em>{magnitudeHint(optimization.totalProfit)} ISK</em></div><div><small>已使用 LP</small><b>{formatNumber(optimization.usedLp)}</b><em>{magnitudeHint(optimization.usedLp)} LP</em></div><div><small>剩余 LP</small><b>{formatNumber(optimization.remainingLp)}</b><em>{magnitudeHint(optimization.remainingLp)} LP</em></div><span>{optimization.exact ? "精确组合" : "大额 LP：尾部精确优化"}</span></div><div className="optimization-lines">{optimization.lines.map(line => <div key={line.offer.offer_id}><strong>{line.offer.item_name}</strong><b>× {formatNumber(line.count)}</b><span>消耗 {formatNumber(line.offer.lp_cost * line.count)} LP<small>{magnitudeHint(line.offer.lp_cost * line.count)}</small></span><span>利润 {formatNumber(line.offer.minProfit! * line.count)} ISK<small>{magnitudeHint(line.offer.minProfit! * line.count)}</small></span></div>)}</div></section>}</>;
  const implantSetPanel = implantSets.length ? <section className="implant-sets"><div><p className="eyebrow">特殊商品</p><h3>成套脑插市场价值</h3><span>完整 Alpha–Omega 六件套，按六件当前最低卖单价合计</span></div>{implantSets.map(set => <div className="implant-set" key={set.name}><div className="implant-set-title"><strong>{set.name}套（6件）</strong></div><div className="implant-levels">{set.items.map((item, index) => <div key={item.level}><i>{index + 1}</i><span>{item.name}</span><b>{item.sell_price > 0 ? `${formatNumber(item.sell_price)} ISK` : "暂无卖单"}</b><em>{formatNumber(item.lp_cost)} LP</em><button className="market-order-link" onClick={event => openMarket({ typeId: item.type_id, name: item.name }, event.currentTarget)}>市场订单</button></div>)}</div><div className="implant-total"><div className="implant-total-ratio"><span>LP 比例</span><strong>{set.lp_ratio === null ? "—" : `${formatNumber(set.lp_ratio)} ISK/LP`}<small>{set.lp_ratio === null ? "价格未齐全" : "按套装市场总价 ÷ 总 LP"}</small></strong></div><div><span>总价</span><strong>{set.market_value === null ? "价格未齐全" : `${formatNumber(set.market_value)} ISK`}<small>约 {set.market_value === null ? "—" : `${magnitudeHint(set.market_value)} ISK`}</small></strong></div><div className="implant-total-lp"><span>总计 LP</span><strong>{formatNumber(set.total_lp)} LP<small>约 {magnitudeHint(set.total_lp)} LP</small></strong></div></div></div>)}</section> : null;
