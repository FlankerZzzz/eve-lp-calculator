...
export default function Home() {
  ...
  const implantSetPanel = implantSets.length ? <section className="implant-sets"><div><p className="eyebrow">特殊商品</p><h3>成套脑插市场价值</h3><span>完整 Alpha–Omega 六件套，按六件当前最高收单价合计</span></div>{implantSets.map(set => <div className="implant-set" key={set.name}><div className="implant-set-title"><strong>{set.name}套（6件）</strong></div><div className="implant-levels">{set.items.map((item, index) => <div key={item.level}><i>{index + 1}</i><span>{item.name}</span><b>{item.sell_price > 0 ? `${formatNumber(item.sell_price)} ISK` : "暂无卖单"}</b><em>{formatNumber(item.lp_cost)} LP</em><button className="market-order-link" onClick={event => openMarket({ typeId: item.type_id, name: item.name }, event.currentTarget)}>市场订单</button></div>)}</div><div className="implant-total"><div className="implant-total-ratio"><span>LP 比例</span><strong>{set.lp_ratio === null ? "—" : `${formatNumber(set.lp_ratio)} ISK/LP`}<small>{set.lp_ratio === null ? "价格未齐全" : "按套装收单总价 ÷ 总 LP"}</small></strong></div><div><span>总价</span><strong>{set.market_value === null ? "价格未齐全" : `${formatNumber(set.market_value)} ISK`}<small>约 {set.market_value === null ? "—" : `${magnitudeHint(set.market_value)} ISK`}</small></strong></div><div className="implant-total-lp"><span>总计 LP</span><strong>{formatNumber(set.total_lp)} LP<small>约 {magnitudeHint(set.total_lp)} LP</small></strong></div></div></div>)}</section> : null;
