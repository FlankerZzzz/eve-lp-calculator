...
function writeChunk(rows) {
  ...
  try {
    ...
    db.prepare("UPDATE sync_jobs SET processed=processed+?,phase='当前市场订单',last_response=?,updated_at=? WHERE kind='orders' AND status='running'").run(rows.length, JSON.stringify({ message: "独立同步器运行中", writerBatch: rows.length, concurrency: ORDER_CONCURRENCY }), now);
    db.exec("COMMIT");
...
async function runBatch(typeIds, runStartedAt) {
  ...
  let completed = 0;
  const launch = () => {
    while (active < ORDER_CONCURRENCY && cursor < typeIds.length) {
      const typeId = typeIds[cursor++];
      active += 1;
      (async () => {
        try {
          queue.push({ typeId, now: new Date().toISOString(), summary: summarize(await fetchOrders(typeId)) });
          failedUntil.delete(typeId);
        } catch (error) {
          failedUntil.set(typeId, Date.now() + 300000);
          console.error("[order-sync] type " + typeId + " failed; retry after 5m", error);
        } finally {
          active -= 1;
          completed += 1;
        }
  ...
  launch();
  while (completed < typeIds.length || active > 0) {
    if (queue.length) writeChunk(queue.splice(0, ORDER_CONCURRENCY));
    launch();
    await sleep(100);
  }
  while (queue.length) writeChunk(queue.splice(0, ORDER_CONCURRENCY));
...
console.log("[order-sync] 常驻调度器启动：滑动窗口 " + ORDER_CONCURRENCY + " 并发，单 SQLite writer");
while (true) {
  try {
    ...
    const rows = db.prepare(`SELECT type_id FROM item_types WHERE ${visible} AND (order_synced_at IS NULL OR order_synced_at < ?) ORDER BY COALESCE(order_synced_at,'') LIMIT 100`).all(job.run_started_at).filter(row => (failedUntil.get(row.type_id) || 0) <= Date.now());
    if (!rows.length) {
      if ([...failedUntil.values()].some(retryAt => retryAt > Date.now())) {
        await sleep(1000);
        continue;
      }
      db.prepare("UPDATE sync_jobs SET status='complete',phase='完成',remaining=0,updated_at=? WHERE kind='orders'").run(new Date().toISOString()); await sleep(1000); continue;
    }
